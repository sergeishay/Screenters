...
export class GeneralStore {
  ...
  @action async getUserById(userId) {
    let getUserById = await axios.get(
      `http://localhost:8080/api/users/${userId}`
    )
    return getUserById
  }
  @action async getCreatorById(creatorId) {
    let getCreatorById = await axios.get(
  ...
  @action async deleteShow(showId, eventId) {
    ...
    if (deleteShow) {
      let deleteShowIndex = this.singleEvent.shows.findIndex(
        show => parseInt(show.id) === parseInt(showId)
      )
  ...
  @action async getAllCreators() {
    let getAllCreators = await axios.get(
      `http://localhost:8080/api/creators?isEvents=1&isShows=1`
    )
  ...
  @action async deleteEvent(eventId) {
    let deleteEvent = await axios.delete(
      `http://localhost:8080/api/events?eventId=${eventId}`
    )

    let deleteEventFromList = this.listOfEvents.listOfEvents.findindex(
      deleteId => deleteId.id === eventId
    )
