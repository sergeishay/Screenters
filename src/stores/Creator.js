...
export class Creator extends User {
    ...
    @action async addNewEvent(eventData) {
        let addEvent = await axios.post(`http://localhost:8080/api/events/event`, eventData )
    ...
    @action async addShow(eventId, creatorId, showData) {
        let addNewShow = await axios.post(`http://localhost:8080/api/events/${eventId}/${creatorId}`, { showData })

    }
    @action async deleteShow(eventId, creatorId, showId) {
        let deleteShow = await axios.delete(`http://localhost:8080/api/events/${eventId}/${creatorId}`, showId)

    }
    @action async updateShow(eventId, creatorId, showData) {
        let updateShow = await axios.put(`http://localhost:8080/api/events/${eventId}/${creatorId}`, { showData })
