...
const Step = props => {
  const { brand, duotone, fab, fad, fal, far, form, icon, light, regular, stepName, tag: Tag, vertical } = props;

  const iconPrefix =
    regular || far ? 'far' : light || fal ? 'fal' : duotone || fad ? 'fad' : brand || fab ? 'fab' : 'fa';
